...
@interface LynxRecorderEntranceViewController ()
...
@property(nonatomic, strong) id<LynxRecorderActionCallback> actionCallback;

@end

@implementation LynxRecorderEntranceViewController
...
+ (BOOL)entranceLynxRecorderView:(id<LynxRecorderActionCallback>)actionCallback {
  ...
  for (NSString *arg_str in argvs) {
    if (arg_str.length > 0 &&
        [arg_str hasPrefix:[[LynxRecorderEnv sharedInstance] lynxRecorderUrlPrefix]]) {
      LynxRecorderEntranceViewController *testbenchVC = [LynxRecorderEntranceViewController new];
      testbenchVC.url = arg_str;
      testbenchVC.actionCallback = actionCallback;
      UINavigationController *navigationVC = [LynxRecorderUIHelper getTopNavigationController];
...
- (void)openTargetUrl:(NSString *)sourceUrl {
  ...
  if ([source.scheme isEqualToString:[[LynxRecorderEnv sharedInstance] lynxRecorderUrlSchema]]) {
    if ([[source host] isEqualToString:[[LynxRecorderEnv sharedInstance] lynxRecorderUrlHost]]) {
      NSArray *query = [[source query] componentsSeparatedByString:@"&"];

      LynxRecorderViewController *tbVC = [LynxRecorderViewController new];
      [tbVC registerLynxRecorderActionCallback:self.actionCallback];
